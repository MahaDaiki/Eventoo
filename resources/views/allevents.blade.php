...
<form class="events-search" method="post" action="{{ route('search.events') }}">
    @csrf
    <div class="container">
        <div class="row">
...
<div class="container">
    <div class="row events-list">
            ...
            @forelse ($filteredEvents ?? $all as $event)
             <div class="col-12 col-lg-6 single-event card mt-2" >     
           <figure class="events-thumbnail mt-2">
                @foreach ($event->getMedia() as $mediaItem)
                <img   src="{{ $mediaItem->getUrl() }}"  class="card-img-top object-cover img-fluid max-width img-thumbnail" alt="Event Image" style="width: 100%; height: 200px;" >
            @endforeach
            </figure>
            <div class="event-content-wrap">
                <header class="entry-header flex justify-content-between ">
                    <div>
                        <h2 class="entry-title"><i class="bi bi-arrow-return-right"><a href="#"></i>{{ $event->title }}</a></h2>

                        <div class="event-location"><a href="#"><i class="bi bi-geo-alt-fill"></i>{{ $event->location }}</a></div>

                        <div class="event-date"><i class="bi bi-hourglass-split"></i>{{ $event->time }}</div>
                        <div class="event-date"><i class="bi bi-clock-history"></i>{{ $event->duration }}</div>
                        <div class="event-date"><i class="bi bi-ticket-fill"></i>{{ $event->disponible_tickets }}</div>
                    </div>

                    <div class="event-cost flex justify-content-center align-items-center" >
                      <a href="{{ route('event.details', ['id' => $event->id]) }}">See more</a>
                    </div>
                </header>
